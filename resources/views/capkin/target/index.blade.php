<x-app-layout>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Daftar Capaian Kinerja Satker</h3>
            <div class="card-tools">
                <a href="{{ route('capaian-kinerja.target.create') }}" class="btn btn-primary btn-sm">
                    <i class="fas fa-plus"></i> Tambah Capaian Kinerja
                </a>
            </div>
        </div>
        <div class="card">
            <div class="card-body table-responsive p-0">
                <table id="capaian-table" class="table table-hover">
                    ...
                    <tbody>
                        @foreach ($daftar_target_kinerja_satker as $target_kinerja_satker)
                            <tr>
                                <td class="text-center">{{ $loop->iteration }}.</td>
                                <td class="text-center">{{ $target_kinerja_satker->indikator }}</td>
                                <td class="text-center">{{ $target_kinerja_satker->target }}</td>
                                <td class="text-center">{{ $target_kinerja_satker->satuan }}</td>

                                <td class="text-center">
                                    <a href="{{ route('capaian-kinerja.target.edit', $target_kinerja_satker->id) }}"
                                        class="btn btn-sm btn-primary" title="Edit">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                    <button type="button" class="btn btn-sm btn-danger" title="Hapus"
                                        onclick="deleteConfirm('{{ route('capaian-kinerja.target.destroy', $target_kinerja_satker->id) }}')">

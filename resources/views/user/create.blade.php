            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('username') is-invalid @enderror"
                        id="username-text" name="username" placeholder="Username" value="{{ old('username') }}"
                        required>
                        @error('username')
                            <div class="invalid-feedback">
                                {{ $message }}
                ...
                <div class="form-group">
                    ...
                    <select id="satkerSelect" class="form-control select2bs4 @error('satker_id') is-invalid @enderror"
                        ...
                        @foreach ($satkers as $satker)
                            <option {{ old('satker_id') == $satker->id ? 'selected' : '' }} value='{{ $satker->id }}'>{{ $satker->name }}</option>
                        @endforeach
                    </select>
                    @error('satker_id')
                        <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <select id="roleSelect" class="form-control select2bs4 @error('role') is-invalid @enderror"
                        ...
                        <option {{ old('role') == 'admin-provinsi' ? 'selected' : '' }} value="admin-provinsi">admin-provinsi</option>
                        <option {{ old('role') == 'admin-satker' ? 'selected' : '' }} value="admin-satker">admin-satker</option>
                        <option {{ old('role') == 'approver' ? 'selected' : '' }} value="approver">approver</option>
                        <option {{ old('role') == 'supervisor' ? 'selected' : '' }} value="supervisor">supervisor</option>
                        <option {{ old('role') == 'evaluator' ? 'selected' : '' }} value="evaluator">evaluator</option>
                        <option {{ old('role') == 'supervisor-akip' ? 'selected' : '' }} value="viewer">supervisor-akip</option>
                        <option {{ old('role') == 'operator-kinerja' ? 'selected' : '' }} value="operator-kinerja">operator-kinerja</option>

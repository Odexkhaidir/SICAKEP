<x-app-layout>
                    ...
                    <div class="form-group col-4">
                        <select id="month" class="form-control col-sm-6 select2bs4" name="month">
                            ...
                            @foreach ($months as $month)
                                <option {{ $this_month == $month->id ? 'selected' : '' }} value="{{ $month->id }}"> {{ $month->name }} </option>
                            @endforeach
                        </select>
                        <div class="help-block"></div>
                    </div>
    ...
    @if ($status < 2)
        <div class="alert alert-warning alert-dismissible">
            ...
            Penilaian bulan {{ $this_month }} tahun {{ $this_year }} masih belum tersedia.
        </div>
    @endif
                    ...
                    @foreach ($evaluations as $evaluation)
                        <tr>
                            ...
                            <td style="text-align:center" id="{{ 'realization_score_' . $evaluation['code'] }}">
                                {{ $evaluation['realization_score'] != null ? round($evaluation['realization_score'], 2) : '-' }}</td>
                            <td style="text-align:center" id="{{ 'time_score_' . $evaluation['code'] }}">
                                {{ $evaluation['time_score'] != null ? round($evaluation['time_score'], 2) : '-' }}</td>
                            <td style="text-align:center" id="{{ 'quality_score_' . $evaluation['code'] }}">
                                {{ $evaluation['quality_score'] != null ? round($evaluation['quality_score'], 2) : '-' }}</td>
                            <td style="text-align:center" id="{{ 'average_score_' . $evaluation['code'] }}">
                                {{ $evaluation['average_score'] != null ? round($evaluation['average_score'], 2) : '-' }}</td>
                            <td style="text-align:center" id="{{ 'rank_' . $evaluation['code'] }}">
                                {{ $evaluation['rank'] }}</td>
                                <td><a href="/evaluation/detail/{{$this_year .'/'. $this_month.'/'.$evaluation['satker_id']}}" ><span class="badge bg-warning"> detail </span></a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>

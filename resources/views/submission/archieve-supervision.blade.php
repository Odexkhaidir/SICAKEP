<x-app-Layout>
        ...
        <div class="card-body">
                ...
                <div class="col-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="year" placeholder="Deskripsi Permindok"
                            name="year" disabled value="{{ $submission->permindok->description }}">
                    </div>
                ...
                <div class="col-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="month" placeholder="Waktu Permindok"
                            name="month" disabled
                            value="{{ $submission->permindok->start_date . ' - ' . $submission->permindok->end_date }}">
                    </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="satker" placeholder="Satuan Kerja" name="satker"
                    disabled value="{{ $submission->satker->name }}">
            </div>
        </div>
    </div>
                ...
                <tbody>
                    @foreach ($archieves as $archieve)
                        <tr>
                            ...
                            <td>{{ $archieve->document->name }}</td>
                            <td class="project-actions text-center">
                                <a class="btn btn-{{$archieve->link ? 'warning' : 'default'}} btn-sm" href="{{ $archieve->link }}" target="{{$archieve->link ? 'blank' : ''}}" disabled="{{$archieve->link ? '' : 'disabled'}}">
                                    <i class="fas fa-link">
                                    </i>
                                    Link
                                </a>
                            ...
                            <td class="text-center">
                                @switch($archieve->status)
                                    @case('uploaded')
                                        <span class="badge bg-success"> {{ $archieve->status }} </span>
                                    @break

                                    @case('submit')
                                        <span class="badge bg-primary"> {{ $archieve->status }} </span>
                                    @break

                                    @case('kosong')
                                        <span class="badge bg-warning"> {{ $archieve->status }} </span>
                                    @break

                                    @default
                                        <span class="badge bg-danger"> belum upload </span>
                                @endswitch
                                <a onclick="archieveStatusForm('{{ route('archieve.updatestatus', $archieve->id) }}', '{{ $archieve->status }}')"
                                    class="btn btn-default btn-xs" href="#"><i class="fas fa-pencil-alt"></i>
                                </a>
                            </td>

                            <td class="text-center"><a href="{{ route('submission.audit.supervision', $archieve->id) }}"><span class="badge bg-warning">pemeriksaan</span></a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <div class="modal fade" id="updateStatus-modal">
            ...
            <form id="status-form" action="" method="post">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Status Dokumen</h5>
    ...
    <x-slot name="script">
        ...
        <script>
            ...
            function archieveStatusForm(url, link) {
                $('#status-form').attr('action', url);
                $('#status-text').attr('value', link);
